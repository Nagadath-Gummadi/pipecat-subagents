...
from pipecat.frames.frames import LLMContextFrame, LLMMessagesAppendFrame, TextFrame
...
class TestAgentContextProcessor(unittest.IsolatedAsyncioTestCase):
    async def test_prepends_system_messages_to_context(self):
        ...
        processor = AgentContextProcessor(system_messages=system_messages)
    ...
    async def test_messages_append_frame_adds_to_context(self):
        """LLMMessagesAppendFrame adds messages to agent context."""
    ...
    async def test_messages_append_no_run_llm(self):
        """LLMMessagesAppendFrame with run_llm=False does not push LLMContextFrame."""
        system_messages = [{"role": "system", "content": "Be helpful."}]
        processor = AgentContextProcessor(system_messages=system_messages)
        pipeline = Pipeline([processor])
